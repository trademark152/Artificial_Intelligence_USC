...
class FIFOQueue(Queue):
    ...
    # to know the length of the queue
    def __len__(self):
        return len(self.A) - self.start

    # to append multiple items to the queue
    def extend(self, items):
        self.A.extend(items)

    # to pop an item based on FIFO
    def pop(self):
        # pop the first elemenet (last in)
        e = self.A[self.start]
